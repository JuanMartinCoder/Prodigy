
import Link from "next/link";

import localFont from "next/font/local"

import { Medal } from "lucide-react"
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button";



const headingFont = localFont({
    src: "../../public/fonts/Righteous-Regular.ttf"
})

export default function MarketingPage () {
    return (
        <div className="flex items-center justify-center flex-col">
            <div className={cn(
                "flex items-center justify-center flex-col",
                headingFont.className
                )}>
                <div className="mb-4 flex items-center border shadow-sm p-4 bg-amber-100 
                            text-amber-700 rounded-full uppercase">
                    <Medal className="h-6 w-6 mr-2"/>
                    N° 1 task managment
                </div>
                <h1 className="text-3xl md:text-6xl text-center 
                text-neutral-800 mb-6">
                    Prodigy helps team move</h1>
                <div className="text-3xl md:text-6xl bg-gradient-to-r 
                from-emerald-400 to-green-600 text-white px-4 p-2 
                rounded-md pb-4 w-fit">
                    work forward.
                </div>
            </div>
            <div className="text-sm md:text-xl text-neutral-400 mt-4
            max-2-xs md:max-w-2xl text-center mx-auto">
            From task assignment to deadline tracking, ensuring seamless execution and timely delivery. 
            </div>
            <Button className="mt-6" size="lg">
                <Link href="/sign-up">Get Prodigy for free</Link>
            </Button>
        </div>
    );
}